import {type cell} from './gameBoard'
...
export function checkWordsOnBoard(gameBoard: Array<Array<Array<cell<number, string>>>>): boolean {
    const words: string[] = [];
    ...
    for (let row = 0; row < gameBoard.length; row++) {
      for (let col = 0; col < gameBoard[row][0].length; col++) {
        ...
        // Check right
        for (let j = col; j < gameBoard[row][0].length && getCharAt(row, j) !== ""; j++) {
